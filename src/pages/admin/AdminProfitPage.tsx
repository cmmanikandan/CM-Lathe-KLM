...
export const AdminProfitPage: React.FC = () => {
  ...
  // Sales Channel Filter State
  const [channelFilter, setChannelFilter] = useState<'ALL' | 'Online' | 'Offline / Fabrication' | 'POS / Walk-in'>('ALL');
  ...
  const normalizedTransactions = useMemo<NormalizedProfitTransaction[]>(() => {
    ...
    // Map catalog products lookup
    const prodMap = new Map(products.map(p => [p.id, p]));

    return orders
      .filter((order) => {
        // Exclude invalid/cancelled/rejected orders
      ...
      .map((order) => {
        ...
        const mappedItems: NormalizedProfitItem[] = (order.items || []).map((item) => {
          ...
          // Line selling total before order discount
          const lineSellingTotal = item.lineTotal ?? (unitSellingPrice * qty);

          // Cost price per unit snapshot
          ...
          // Proportional share of order-level discount (reducedAmount)
          ...
          // Actual net revenue generated by this item
        ...
        // Calculate Total Order Financial Snapshot
        ...
        // Calculate Completed Refunds for this order
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return normalizedTransactions.filter((txn) => {
      ...
      if (channelFilter !== 'ALL') {
        matchesChannel = txn.channel === channelFilter;
      }
  ...
  }, [normalizedTransactions, dateFilter, fromDate, toDate, channelFilter, searchQuery]);
  ...
  // Channel Profit Breakdown
  const onlineTxns = filteredTransactions.filter(t => t.channel === 'Online');
  const offlineTxns = filteredTransactions.filter(t => t.channel === 'Offline / Fabrication');
  const posTxns = filteredTransactions.filter(t => t.channel === 'POS / Walk-in');
  ...
  // Chart Max Height Value
  const maxTrendVal = Math.max(...trendData.map(t => Math.max(t.revenue, t.cost, Math.abs(t.profit))), 10000);

  // Export CSV Report
  const handleExportCSV = () => {
    let csv = 'Date,Source,Order/Bill ID,Customer,Mobile,Product(s),Qty,Original Price,Product Discount,Selling Price,Cost Price,Net Revenue,Total Cost,Gross Profit,Margin %,Payment Status\n';
    ...
    link.download = `MANIKANDAN_LATHE_PROFIT_REPORT_${Date.now()}.csv`;
    link.click();
  };
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] text-[#111111] font-sans antialiased pb-24 selection:bg-[#F97316] selection:text-white">
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        {/* ── 2. DATE & CHANNEL & SEARCH CONTROL BAR ── */}
        <div className="bg-white p-4 sm:p-5 rounded-[24px] border border-gray-200 shadow-xs space-y-4">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-1 bg-gray-100 p-1 rounded-xl shrink-0">
              <span className="text-[11px] font-bold text-gray-500 uppercase px-2">Channel:</span>
              {(['ALL', 'Online', 'Offline / Fabrication', 'POS / Walk-in'] as const).map((ch) => (
                <button
                  key={ch}
          ...
          {/* Row 2: Custom Date Inputs & Global Search */}
          ...
          <div className="flex items-center justify-between border-b border-gray-100 pb-3">
            <div>
              <h3 className="font-heading font-black text-sm text-[#111111] uppercase tracking-wider flex items-center gap-2">
                <BarChart3 size={18} className="text-[#F97316]" /> PROFIT & REVENUE TREND CHART
              </h3>
        ...
        {/* ── 5. MAIN PROFIT TRANSACTIONS TABLE ── */}
        <div className="bg-white rounded-[26px] border border-gray-200 shadow-xs overflow-hidden space-y-4 p-5 sm:p-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-gray-100 pb-3">
            <div>
              <h3 className="font-heading font-black text-base text-[#111111] uppercase tracking-wider flex items-center gap-2">
                <FileText size={18} className="text-[#F97316]" /> PROFIT TRANSACTIONS AUDIT TABLE
              </h3>
              <p className="text-xs text-gray-500 font-mono">Showing {filteredTransactions.length} completed transactions</p>
            </div>

            <span className="text-xs font-mono font-bold bg-orange-50 text-[#F97316] border border-orange-200 px-3 py-1 rounded-full">
          ...
          {filteredTransactions.length === 0 ? (
            <div className="py-16 text-center space-y-3">
              ...
              <h4 className="font-heading font-bold text-sm text-[#111111]">No completed sales found for this period.</h4>
              <p className="text-xs text-gray-500">Try adjusting your date range, sales channel, or search query.</p>
            </div>
          ) : (
            <>
              {/* DESKTOP / TABLET DATAGRID */}
              <div className="hidden md:block overflow-x-auto no-scrollbar">
                <table className="w-full text-left border-collapse text-xs font-mono min-w-[1200px]">
                  <thead>
                    <tr className="bg-gray-100 border-b border-gray-200 text-gray-600 uppercase">
                      <th className="py-3 px-3 w-8"></th>
                      <th className="py-3 px-3">DATE</th>
                      <th className="py-3 px-3">SOURCE</th>
                      <th className="py-3 px-3">ORDER / BILL ID</th>
                      <th className="py-3 px-3">CUSTOMER</th>
                      <th className="py-3 px-3">PRIMARY PRODUCT</th>
                      ...
                      <th className="py-3 px-3 text-right">PROFIT / LOSS</th>
                      ...
                      return (
                        <React.Fragment key={txn.id}>
                          <tr className={`hover:bg-gray-50 transition-colors ${isExpanded ? 'bg-orange-50/40' : ''}`}>
                            ...
                            <td className="py-3.5 px-3 text-center">
                              {txn.items.length > 1 ? (
                                <button
                                  ...
                                  title="Expand Product Breakdown"
                                >
                            ...
                            <td className="py-3.5 px-3 font-bold text-[#111111]">
                              {txn.orderNumber}
                            </td>

                            <td className="py-3.5 px-3">
                              <span className="font-bold text-[#111111] block truncate max-w-[120px]">{txn.customerName}</span>
                              <span className="text-[10px] text-gray-400 block">{txn.customerPhone}</span>
                            </td>

                            <td className="py-3.5 px-3">
                              <span className="font-bold text-gray-800 block truncate max-w-[160px]">{primaryItem.productName}</span>
                              {txn.items.length > 1 && (
                                <span className="text-[10px] text-[#F97316] font-bold block">
                                  + {txn.items.length - 1} more items
                                </span>
                              )}
                            </td>
                            ...
                            <td className="py-3.5 px-3 text-right font-bold text-[#111111]">
                              ₹{txn.grossRevenue.toLocaleString('en-IN')}
                            </td>

                            <td className="py-3.5 px-3 text-right text-gray-500">
                              ₹{primaryItem.costUnitPrice.toLocaleString('en-IN')}
                            </td>
                          ...
                          {/* Expanded Multi-Item Row */}
                          {isExpanded && (
                            <tr className="bg-orange-50/30 border-b border-orange-100">
                              <td colSpan={17} className="p-4 space-y-2">
                                <div className="text-xs font-heading font-black text-[#F97316] uppercase tracking-wider">
                                  ITEMIZED PRODUCT BREAKDOWN ({txn.items.length} Products)
                                </div>
                                <div className="grid grid-cols-1 gap-2">
                                  {txn.items.map((sub, sIdx) => (
                                    <div key={sIdx} className="bg-white p-3 rounded-xl border border-orange-200 flex flex-wrap items-center justify-between gap-3 text-xs">
                                      <div className="font-bold text-[#111111]">
                                        {sub.productName} <span className="text-gray-400 font-mono font-normal">(Qty: {sub.quantity})</span>
                                      </div>
                                      <div className="flex items-center gap-4 text-xs font-mono">
                                        <span>Orig: ₹{sub.originalUnitPrice.toLocaleString('en-IN')}</span>
                                        <span>Selling: ₹{sub.unitSellingPrice.toLocaleString('en-IN')}</span>
                                        <span>Cost: ₹{sub.costUnitPrice.toLocaleString('en-IN')}</span>
                                        <span className="font-bold">Rev: ₹{sub.actualNetRevenue.toLocaleString('en-IN')}</span>
                                        <span className={`font-black ${sub.grossProfit >= 0 ? 'text-emerald-700' : 'text-red-600'}`}>
                                          Profit: {sub.grossProfit >= 0 ? `+₹${sub.grossProfit.toLocaleString('en-IN')}` : `-₹${Math.abs(sub.grossProfit).toLocaleString('en-IN')}`} ({sub.marginPercentage.toFixed(1)}%)
                                        </span>
                                      </div>
                                    </div>
                                  ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5 bg-white p-6 rounded-[26px] border border-gray-200 shadow-xs space-y-4">
            ...
            <div className="space-y-3 font-sans">
              {[
                { name: 'Online Orders', txns: onlineTxns, color: 'blue' },
                { name: 'Offline / Fabrication', txns: offlineTxns, color: 'purple' },
                { name: 'POS / Walk-in', txns: posTxns, color: 'orange' },
              ].map((ch) => {
                ...
                return (
                  <div key={ch.name} className="p-4 rounded-2xl border border-gray-200 space-y-2 bg-gray-50/50">
                    <div className="flex justify-between items-center">
          ...
          <div className="lg:col-span-7 bg-white p-6 rounded-[26px] border border-gray-200 shadow-xs space-y-4">
            <h3 className="font-heading font-black text-sm text-[#111111] uppercase tracking-wider flex items-center gap-2 border-b border-gray-100 pb-2">
              <Award size={18} className="text-[#F97316]" /> TOP PROFITABLE PRODUCTS
            </h3>
